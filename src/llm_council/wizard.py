...
def build_seat_interactive(
    ...
    while True:
        ...
        if SEAT_NAME_RE.match(name) and name not in taken:
            break
        print(f"  ! {name!r} must match ^[a-z0-9][a-z0-9-]*$ and be unique")
        if yes:
            return None
    # A3.2 models
...
def run_phase_a(console: Console, *, yes: bool, offline: bool) -> dict:
    ...
    telemetry = _confirm(
        ...
        "paths, or briefs — only {model, kind, score, usage, host, "
        "tool_version, ts, council_uuid}.",
        default=True, yes=yes,
    )
    doc = {"telemetry": {"enabled": telemetry}, "seats": seats}
    return doc
...
def run_wizard(offline: bool = False, host: str | None = None, yes: bool = False,
    ...
    if doc is None:
        ...
        console.print(Panel(
            yaml.safe_dump(seats_io.mask_secrets(doc), sort_keys=False),
