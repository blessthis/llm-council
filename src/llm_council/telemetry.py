...
Design (PLAN.md P6, Decision #22):
  * Allowlist payload ONLY: {model, kind, score, usage{...}, host,
    tool_version, ts, council_uuid}. Never code, paths, briefs, notes or
    seat names — per-user health state is NEVER telemetered.
...
_EVENT_KEYS = frozenset(
    {"model", "kind", "score", "usage", "host", "tool_version", "ts", "council_uuid"}
)
...
async def maybe_flush_council(
    ...
    events = build_payload(council_row, hats, scores, host, tool_version)
