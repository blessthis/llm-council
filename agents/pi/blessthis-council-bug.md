...
# Conductor model: any LIVE model works — the conductor only orchestrates + verifies; the SEAT
# roster (the actual analysis) is chosen separately by the server from the healthy roster.
# Pinned to kimi-k3 while claude providers (opus/sonnet/fable) are exhausted (503 auth_unavailable).
# TODO: flip back to `model: opus` when claude recovers (backup of the original opus pin is at
# bug-council.md.bak-opus-pin).
