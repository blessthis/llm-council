...
- **invoke → CLI:** `pi --mode json --no-extensions --no-skills --no-prompt-templates --no-context-files -p {prompt} --model {model}`. pi has NO `--add-dir` flag — the working dir is the subprocess **cwd** (the runner passes `workdir` as `cwd` to `create_subprocess_exec`). Safety flags keep the headless seat free of host-side extensions/skills/context (`--offline` also available). No `--tools` allowlist by default (it would cut off the user's MCP servers/extensions); add `--tools <allowlist>` to sandbox a seat. **No `--no-session`** — sessions are required for council_ask.
